...
<body class="is-preload">
                ...
                <div class=" ">
                    ...
                    <div id="custom-tabs" class="custom-tabs bg-dangerx d-flex">
                        <div class="d-flex justify-content-between align-items-center ps-5 pe-5 pt-3">
                            <div class="d-flex p-1">
                                <a href="user-archives.php" class="text-decoration-none btn border-0 text-dark d-flex align-items-center justify-content-between ms-1 me-1 p-2 border-start border-5 border-primary rounded-0 bg-primary bg-opacity-25">
                                    <i class="fa-solid fa-folder icon p-1"></i>
                                    <h6 class="fw-mediumm p-1 fsz">Equipment Records</h6>
                                </a>
                                <a href="user-archives-inv.php" class="text-decoration-none btn border-0 text-dark d-flex align-items-center justify-content-between ms-1 me-1 p-2 opacity-75 border-start border-2 rounded-0">
                    ...
                    <div class="container-fluid ps-3 pe-3">
                        <div class="table-div h-100x borderx ps-5 pe-5 pt-3 pb-5 overflow-auto">
                            <table id="userarchive-tbl" class="table pt-3">
                                <thead class="text-center border-bottom border-dark fw-bold">
                                    <tr>
                                        <th hidden class="th-bg">ID</th>
                                        <th class="th-bg">Equipment</th>
                                        <th class="th-bg text-end">...</th>
                                    </tr>
                                </thead>
                                <tbody class="text-center">
                                    <?php
                                    $eq = mysqli_query($connect, "SELECT * FROM equipments WHERE archive=1 AND userID='$user'");
                                    if (mysqli_num_rows($eq) > 0) {
                                        while ($row = mysqli_fetch_assoc($eq)) {
                                    ?>
                                            <tr class="tr">
                                                <td hidden class=""><?php echo htmlspecialchars($row['equipment_id'], ENT_QUOTES, 'UTF-8'); ?></td>
                                                <td class=""><?php echo htmlspecialchars($row['equip_name'], ENT_QUOTES, 'UTF-8'); ?></td>
                                                <td class="text-center d-flex justify-content-end">
                                                    <div class="d-flex justify-content-between">
                                                        <div class="m-1">
                                                            <button type="button" class="btn btn-success fsz d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#restore<?php echo $row['equipment_id']; ?>"><i class="fa-solid fa-arrow-up-from-bracket m-1"></i>Restore</button>
                                                        </div>
                                                        <div class="m-1">
                                                            <button type="button" class="btn btn-danger fsz d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#delete<?php echo $row['equipment_id']; ?>"><i class="fa-solid fa-trash-can m-1"></i>Delete</button>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                                            ...
                                            <!--DELETE MODAL -->
                                            <div class="modal fade" id="delete<?php echo $row['equipment_id']; ?>" tabindex="-1" aria-labelledby="" aria-hidden="true">
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="card text-center border-0 m-5 d-flex justify-content-center">
                                                            ...
                                                            <div class="d-flex justify-content-center">
                                                                ...
                                                                <button type="button" name="deleteEQ" id="<?php echo $row['equipment_id']; ?>" class="btn btn-danger deleteEQ">YES</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <!--RESTORE MODAL -->
                                            <div class="modal fade" id="restore<?php echo $row['equipment_id']; ?>" tabindex="-1" aria-labelledby="" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="card text-center border-0 m-5 d-flex justify-content-center">
                                                            <div class="mb-4">
                                                            ...
                                                            <div class="d-flex justify-content-center">
                                                                <button type="button" class="btn btn-warning me-3" data-bs-dismiss="modal" aria-label="NO">NO</button>
                                                                <button type="button" name="restoreEQ" id="<?php echo $row['equipment_id']; ?>" class="btn btn-success me-2 restoreEQ">YES</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            //restore button
            $('.restoreEQ').on('click', function() {
                var restoreEQ = $(this).attr('id');
                $.ajax({
                    type: "POST",
            ...
            //delete button
            $('.deleteEQ').on('click', function() {
                var deleteEQ = $(this).attr('id');
                $.ajax({
                    type: "POST",
                    url: "../config/archive.php",
                    data: {
