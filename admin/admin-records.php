<?php
// ADMIN: MAINTENANCE RECORDS / REPAIR RECORDS Interface
					...
					<div id="custom-tabs" class="custom-tabs bg-dangerx d-flex">
						<div class="d-flex justify-content-between align-items-center  pt-3 ps-5 pe-5">
							<div class="d-flex p-1">
								<a href="admin-records.php" class="text-decoration-none btn border-0 text-dark d-flex align-items-center justify-content-between ms-1 me-1 p-2 border-start border-5 border-primary rounded-0 bg-primary bg-opacity-25">
									<i class="fa-solid fa-screwdriver-wrench icon p-1"></i>
									<h6 class="fw-mediumm p-1 fsz">Troubleshoot and Repair / Installation</h6>
								</a>
								<a href="rac-records.php" class="text-decoration-none btn border-0 text-dark d-flex align-items-center justify-content-between ms-1 me-1 p-2 opacity-75 border-start border-2 rounded-0">
									<i class="fa-brands fa-intercom fa-rotate-90 icon p-1"></i>
									<h6 class="fw-lighter p-1 fsz">RAC Cleaning Records </h6>
								</a>
							</div>
						</div>
					</div>

					<div class="table-div h-100x borderx ps-3 pe-3 pt-3 pb-3 bg-body">
						<table id="records-tbl" class="table pt-3">
							...
							<tbody class="text-center">
								...
								if (mysqli_num_rows($repairs) > 0) {
									while ($row = mysqli_fetch_assoc($repairs)) { ?>
										<tr class="row1">
											<td hidden class=""><?php echo htmlspecialchars($row['repair_id'], ENT_QUOTES, 'UTF-8'); ?></td>
											<td class=""><?php echo htmlspecialchars($row['repair_location'], ENT_QUOTES, 'UTF-8'); ?></td>
											<td class=""><?php echo htmlspecialchars($row['repair_unit'], ENT_QUOTES, 'UTF-8'); ?></td>
											<td class=""><?php echo htmlspecialchars($row['equipment_device'], ENT_QUOTES, 'UTF-8'); ?></td>
											<td class=""><?php repairType($row['repair_type']); ?></td>
											<td class=""><?php if (!empty($row['start_time']) && $row['start_time'] !== "00:00:00") {
																echo date("h:i A", strtotime($row['start_time']));
															} else {
																echo "";
															} ?></td>
											<td class=""><?php if (!empty($row['finish_time']) && $row['finish_time'] !== "00:00:00") {
																echo date("h:i A", strtotime($row['finish_time']));
															} else {
																echo "";
															} ?></td>
											<td class=""><?php if (!empty($row['date_finish']) && $row['date_finish'] !== "0000-00-00") {
																echo date("F j, Y", strtotime($row['date_finish']));
															} else {
																echo "";
															} ?></td>
											...
											<td class="" class="">
												<div class="dropdown">
													...
													<ul class="dropdown-menu m-0 p-0">
														...
														<li><a type="button" class="list-group-item border-bottom d-flex justify-content-center ripple text-decoration-none p-2 fsz overflow-hidden archiveRepairs" name="archiveRepairs" id="archiveRepairs" data-bs-toggle="modal" data-bs-target="#archiveRecord<?php echo $row['repair_id']; ?>">Archive</a></li>
													</ul>
												</div>
											</td>
										</tr>
													...
													<div class="card text-center border-0 m-5 d-flex justify-content-center">
														...
														<div class="d-flex justify-content-center">
															<form method="post" action="../config/admin-archive.php">
																...
																<input hidden name="repair" id="repair" value="<?php echo $row['repair_id']; ?>">
																<input hidden name="request" id="request" value="<?php echo $row['job_id']; ?>">
																<button name="archiveRepair" id="<?php echo $row['repair_id']; ?>" class="btn btn-success archiveRepair"><i class="fa-solid fa-square-check p-1"></i>Yes</button>
															</form>
														</div>
													</div>
												</div>
